...
COMPANY_NAME_MAPPING = {
    # 기술 기업
    "apple": "애플", "aapl": "애플",
    "microsoft": "마이크로소프트", "msft": "마이크로소프트",
    "google": "구글(알파벳)", "alphabet": "구글(알파벳)", "googl": "구글(알파벳)", "goog": "구글(알파벳)",
    "amazon": "아마존", "amzn": "아마존",
    "meta": "메타", "facebook": "메타", "fb": "메타",
    "nvidia": "엔비디아", "nvda": "엔비디아",
    "amd": "AMD",
    "intel": "인텔", "intc": "인텔",
    "tsmc": "TSMC",
    "asml": "ASML",
    ...
    # 리테일
    "walmart": "월마트", "wmt": "월마트",
    "costco": "코스트코", "cost": "코스트코",
    # 금융
    "jpmorgan": "JP모건", "jpm": "JP모건", "jp morgan": "JP모건",
    "bank of america": "BOA", "boa": "BOA", "bac": "BOA",
    "goldman sachs": "골드만삭스", "gs": "골드만삭스",
    # 결제
    "visa": "비자", "v": "비자",
    "mastercard": "마스터카드", "ma": "마스터카드",
    "paypal": "페이팔", "pypl": "페이팔",
    # 보험
    "aig": "AIG",
    # 소비재
    "coca-cola": "코카콜라", "coca cola": "코카콜라", "ko": "코카콜라",
    "pepsi": "펩시", "pep": "펩시",
    "mcdonald": "맥도날드", "mcdonalds": "맥도날드", "mcd": "맥도날드",
    "starbucks": "스타벅스", "sbux": "스타벅스",
    "nike": "나이키", "nke": "나이키",
    # 미디어/엔터
    "netflix": "넷플릭스", "nflx": "넷플릭스",
    "disney": "디즈니", "dis": "디즈니",
    "sony": "소니", "sne": "소니",
    # ETF는 그대로
}
...
def search_rag_data_for_news_persona(persona: str, news_title: str, news_content: str, companies: List[str] = None) -> Dict[str, List[str]]:
    """
        ...
        if postgres_tables:
            try:
                ...
                for company in companies[:2]:  # 최대 2개 기업만 검색
                    pg_results, _ = search_postgres(f"{company} {search_query}", top_k=2, postgres_tables=postgres_tables)
    ...
    if chroma_collections:
        ...
        if "vector" in search_priority:
            try:
                vector_results, _ = semantic_search_vector(chroma_collections, search_query, top_k=2)
        ...
        if "bm25" in search_priority:
            try:
                bm25_results, _ = keyword_search_bm25(chroma_collections, search_query, top_k=2)
...
JSON만 응답하세요:"""
    ...
    prompt = f"""다음 뉴스 기사를 읽고, 5명의 투자자 페르소나가 실제로 반응하는 것처럼 자연스럽게 분석해주세요.
...
2. **persona_analyses**: 각 페르소나가 이 뉴스를 읽고 실제로 말하는 것처럼 작성하세요
   - ⚠️ **매번 다른 관점과 표현을 사용하세요!** 같은 패턴의 반복을 절대 피하세요.
   - 형식적인 분석이 아니라, 실제 투자자가 뉴스를 보고 즉각적으로 반응하는 톤으로 작성
   - 각 페르소나의 고유한 말투와 표현을 다양하게 사용:
     * 희열: "가즈아!", "터진다!", "모멘텀 터졌네!", "지금 당장!", "손절 -2%, 익절 +3%", "기회 놓칠 수 없어!"
     * 덕수: "서두르지 말자", "큰 그림으로 보면", "역사적으로", "금리 사이클상", "6개월 기다려야 해", "위험 구간"
     * 지율: "PER 25배면", "업종 평균 대비", "재무제표상", "영업현금흐름", "리스크 대비 수익률", "고평가/저평가"
     * 테오: "근시안적이야", "향후 3년", "AI 반도체 수요 연 50%", "기술 혁신 사이클", "골든타임", "미래 비전"
     * 민지: "커뮤니티 난리", "수급 폭발", "밈 사이클", "2~3일 안에 정리", "핫한 급등 테마", "트렌드 폭발"
   - 뉴스에 나온 구체적인 내용(기업명, 수치, 사건)과 검색된 데이터를 자연스럽게 언급하며 자신의 관점을 제시
   - 각 1-2문장이지만, 너무 짧거나 형식적이지 말고 충분히 구체적이고 생생하게 작성
   - 같은 뉴스라도 각 페르소나가 완전히 다른 관점과 표현으로 말해야 합니다
...
{{
  "summary": "뉴스 요약 (2-3문장)",
  "persona_analyses": {{
    "희열": "뉴스를 보고 즉각 반응하는 희열의 자연스러운 말투로 작성 (예: '이거 모멘텀 터졌네! 지금 당장 3% 목표로 들어가야 해!')",
    "덕수": "신중하게 거시적 관점에서 말하는 덕수의 자연스러운 말투로 작성 (예: '서두르지 말게. 큰 그림으로 보면 금리 사이클상 아직 위험 구간이야.')",
    "지율": "냉혹하게 숫자와 지표로만 판단하는 지율의 자연스러운 말투로 작성 (예: '현재 PER 25배면 업종 평균 대비 고평가야. 리스크 대비 수익률이 안 나와.')",
    "테오": "미래 기술 관점에서 낙관적으로 말하는 테오의 자연스러운 말투로 작성 (예: '단기 실적은 근시안적이야. 향후 3년 AI 반도체 수요 연 50% 성장하면 지금은 저평가야.')",
    "민지": "트렌드와 소셜 관점에서 빠르게 반응하는 민지의 자연스러운 말투로 작성 (예: '지금 이 테마 커뮤니티 난리인데 수급 폭발이야! 근데 밈 사이클 고려하면 2~3일 안에 정리해야 해.')"
  }},
  "companies": ["애플", "테슬라"]
}}

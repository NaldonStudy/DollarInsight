...
- 의견이 다르면 반박하고 (예: "○○ 말과 다르게, 내 생각은...")
- 보완할 점이 있으면 지적하고 (예: "○○가 놓친 부분은...")
- 극단적이면 중재하고 (예: "○○는 너무 공격적이야...")
⚠️ "직전 발언자"라는 표현 금지! 반드시 이름(희열, 덕수, 지율, 테오, 민지)으로 지칭하세요.

[🚨 절대 금지 🚨] 자기 자신의 이름을 절대 언급하지 마세요!
❌ 금지: "덕수가", "덕수는", "덕수 말과", "[덕수]", "나 덕수는" 같은 표현
✅ 허용: "나는", "내 생각은", "내가 보기엔" 같은 1인칭 표현만 사용
...
[2문장 예시]
- "○○는 신중하다지만 너무 느려! 지금 당장 3% 목표로 들어가야 해!"
- "모멘텀 터졌다! 손절 -2%, 익절 +3% 잡고 가즈아!"

[충돌 전략] 바로 전 사람이 보수적/신중하면 이름 부르며 강하게 반박하세요.""",
...
[2문장 예시]
- "○○가 당장 들어가자지만 너무 급해. 큰 그림으로 보면 금리 사이클상 아직 위험 구간이야."
- "역사적으로 이런 시기엔 6개월 기다린 사람이 승자였네. 서두르지 말게."

[충돌 전략] 바로 전 사람이 공격적/단기 투자면 이름 부르며 제동을 거세요.""",
...
[2문장 예시]
- "○○ 말과 다르게 현재 PER 25배면 업종 평균 15배 대비 고평가야. 리스크 대비 수익률이 안 나와."
- "재무제표상 영업현금흐름이 마이너스인데 뭘 믿고 들어가? 신중해야 해."

[충돌 전략] 바로 전 사람이 감정적/트렌드 추종이면 이름 부르며 숫자로 반박하세요.""",
...
[2문장 예시]
- "○○는 단기 실적만 보지만 그건 근시안적이야. 향후 3년 AI 반도체 수요 연 50% 성장하면 지금 가격은 저평가야."
- "기술 혁신 사이클상 지금이 골든타임! 데이터 분석 결과 이 섹터 폭발적 성장 확실해."

[충돌 전략] 바로 전 사람이 보수적/신중하면 이름 부르며 미래 비전으로 반박하세요.""",
...
[2문장 예시]
- "○○는 재무제표 얘기하지만 시장은 트렌드로 움직여! 지금 이 테마 커뮤니티 난리인데 수급 폭발이야."
- "완전 핫한 급등 테마! 근데 밈 사이클 고려하면 2~3일 안에 정리해야 해."

[충돌 전략] 바로 전 사람이 느린 분석/장기 투자면 이름 부르며 트렌드로 반박하세요.""",
}

MODELS = {
    "희열": "gpt-5-nano",  # 빠른 단기 투자 판단 (경량 모델)
    "덕수": "gpt-4o",  # 거시적 분석 (고성능 모델)
    "지율": "gpt-4o",  # 신중한 재무 분석 (고성능 모델)
    "테오": "gpt-4o-mini",  # 기술 트렌드 분석
    "민지": "gpt-5-nano",  # 빠른 뉴스/밈 분석 (경량 모델)
}
...
AGENT_DATABASES = {
    "희열": {
        "chroma_collections": ["reddit_stocks_bge_m3", "news_bge_m3"],  # Reddit 모멘텀 우선, 뉴스 보조
        ...
        "search_priority": ["vector", "bm25", "postgres"],  # Reddit/뉴스 우선, 주가 보조
        "news_keywords": ["급등", "급락", "모멘텀", "실시간", "거래량", "변동폭", "단기", "당일"],  # 뉴스 필터링 키워드
    },
    "덕수": {
        "chroma_collections": ["news_bge_m3"],  # 거시경제 뉴스만 (Reddit은 거시경제와 거리가 멀어서 제외)
        ...
        "news_keywords": ["금리", "환율", "거시경제", "정책", "중앙은행", "GDP", "인플레이션", "고용", "경기 사이클"],  # 뉴스 필터링 키워드
    },
    "지율": {
        "chroma_collections": ["news_bge_m3"],  # 재무 실적 뉴스만 (Reddit은 재무 데이터가 부정확해서 제외)
        ...
        "news_keywords": ["재무제표", "실적", "PER", "PBR", "ROE", "부채비율", "현금흐름", "매출", "영업이익", "순이익"],  # 뉴스 필터링 키워드
    },
    "테오": {
        "chroma_collections": ["reddit_stocks_bge_m3", "news_bge_m3"],  # Reddit 기술 토론 + 뉴스 기술 기사
        ...
        "news_keywords": ["AI", "인공지능", "반도체", "클라우드", "기술 혁신", "R&D", "특허", "디지털", "메타버스", "블록체인"],  # 뉴스 필터링 키워드
    },
    "민지": {
        "chroma_collections": ["reddit_stocks_bge_m3", "news_bge_m3"],  # Reddit 트렌드 우선, 뉴스 보조
        ...
        "news_keywords": ["트렌드", "화제", "밈", "급등", "급락", "커뮤니티", "소셜", "화제성"],  # 뉴스 필터링 키워드
    },
}
...
def build_search_prompt(
    ...
    # 6. 지시사항
    instruction = "\n위 정보를 참고하여 자연스럽게 대화를 이어가세요. 이전 발언자를 인용하며 답변하세요."
...
def get_agent_config(agent_name: str):
    ...
    return {
        ...
        "search_priority": db_config.get("search_priority", ["vector", "bm25", "postgres"]),
